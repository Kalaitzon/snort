"""
Training and honest evaluation of the ML models.

Models (aligned with the course material):
  - RandomForest        : supervised, de facto baseline for tabular (lectures 3,4)
  - LogisticRegression  : interpretable supervised baseline (lecture 3)
  - IsolationForest     : unsupervised anomaly detection (lecture 3) -> optional
                          supervised vs anomaly comparison

Principles followed (avoiding the circular labeling / leakage mistakes - lecture 4):
  1. The labels come from event_log (attacker identity), NOT from the features
     themselves.
  2. The scaler and the IsolationForest are trained ONLY on the train split.
  3. The IsolationForest is trained only on benign windows (a clean reference of
     normality), as befits anomaly detection.
  4. All headline metrics refer to the held-out test set.

Produces:
  ml_detector/predictions.csv : predictions of all models for ALL windows
  ml_detector/rf_model.pkl, scaler.pkl, iforest.pkl
  ml_detector/ml_metrics.json : test-set metrics per model
"""
...
def main():
    ...
    rf = RandomForestClassifier(n_estimators=200, max_depth=8,
                                ...
                                random_state=SEED)
    rf.fit(Xtr, ytr)   # tree-based: does not need scaling
    ...
    # Trained ONLY on benign train windows -> "normality".
    Xtr_benign = Xtr_s[ytr == 0]
    contamination = float(ytr.mean())     # ~ attack ratio in the train set
    ...
    if_score_te = -iforest.score_samples(Xte_s)   # larger = more anomalous
    metrics["isolation_forest"] = metrics_block(yte, if_pred_te, if_score_te)

    # --- Predictions for ALL windows (for fusion / case studies) ---
    ...
    # normalize if_score to [0,1] for easy use in the fusion
